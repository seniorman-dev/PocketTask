...
NumberFormat currency() {
  //Locale locale = Localizations.localeOf(context); // Get the app's locale
  //String localeString = locale.toString(); // Convert Locale to string
  var format = NumberFormat.simpleCurrency(locale: Platform.localeName, name: "NGN"); // Define NGN as the currency
  return format;
}


String formatNairaWithIntl(int amount) {
  final format = NumberFormat.currency(
    locale: 'en_NG', // Using Nigerian English locale for consistent formatting
    symbol: '₦',
...
DateTime parseDateTime({required String dateString, required String timeString}) {
  // Parse date string
  DateTime date = DateFormat("yyyy-MM-dd").parse(dateString).toLocal();
...
String calculateDurationBetweenDates(String startDateStr, String endDateStr) {
  ...
  DateTime startDate = dateFormat.parse(startDateStr).toLocal();
  DateTime endDate = dateFormat.parse(endDateStr).toLocal();
...
DateTime convertStringToDateTime(String dateString) {
  try {
     ...
     DateTime dateTime = outputFormat.parse(dateString).toLocal();
...
String convertDateStringToDate(String dateString) {
  ...
  // Convert to DateTime
  DateTime dateTime = DateTime(year, month, day).toLocal();
...
String formatDoubleWithCommas(String number) {
